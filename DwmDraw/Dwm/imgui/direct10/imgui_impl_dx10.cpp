...
static void ImGui_ImplDX10_CreateFontsTexture()
{
    ...
    // Create texture sampler
    {
        ...
        if (bd->pd3dDevice->GetFeatureLevel() == D3D10_FEATURE_LEVEL_9_3) {
            desc.MaxLOD = FLT_MAX;
        }
...
bool    ImGui_ImplDX10_CreateDeviceObjects()
{
    ...
    {
        if (bd->pd3dDevice->CreateVertexShader(imgui_vertex_shader, sizeof(imgui_vertex_shader), &bd->pVertexShader) != S_OK)
        {
            return false;
        }
        ...
        {
			{ POSITION.c_str(), 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D10_INPUT_PER_VERTEX_DATA, 0 },
			{ TEXCOORD.c_str(), 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)IM_OFFSETOF(ImDrawVert, uv),  D3D10_INPUT_PER_VERTEX_DATA, 0 },
			{ COLOR.c_str()   ,    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)IM_OFFSETOF(ImDrawVert, col), D3D10_INPUT_PER_VERTEX_DATA, 0 },
        };

        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, imgui_vertex_shader, sizeof(imgui_vertex_shader), &bd->pInputLayout) != S_OK)
        {
            return false;
        }
    ...
    if (bd->pd3dDevice->CreatePixelShader(imgui_pixel_shader, sizeof(imgui_pixel_shader), &bd->pPixelShader) != S_OK)
    {
        return false;
    }
...
bool    ImGui_ImplDX10_Init(ID3D10Device1* device)
{
    ImGuiIO& io = ImGui::GetIO();
    ...
    io.BackendRendererName = ( "direct10_render" );
    io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.

    TEMP_GUID(IID_IDXGIDevice, 0x54ec77fa, 0x1377, 0x44e6, 0x8c, 0x32, 0x88, 0xfd, 0x5f, 0x44, 0xc8, 0x4c);
    TEMP_GUID(IID_IDXGIAdapter, 0x2411e7e1, 0x12ac, 0x4ccf, 0xbd, 0x14, 0x97, 0x98, 0xe8, 0x53, 0x4d, 0xc0);
