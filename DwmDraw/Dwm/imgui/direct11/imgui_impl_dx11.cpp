...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (bd->pd3dDevice->CreateVertexShader(imgui_vertex_shader, sizeof(imgui_vertex_shader), NULL, &bd->pVertexShader) != S_OK)
        {
            return false;
        }
        ...
        {
            { POSITION.c_str() , 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { TEXCOORD.c_str(), 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)IM_OFFSETOF(ImDrawVert, uv),  D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { COLOR.c_str(),    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)IM_OFFSETOF(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0 },
        };
    ...
    {
        if (bd->pd3dDevice->CreatePixelShader(imgui_pixel_shader, sizeof(imgui_pixel_shader), NULL, &bd->pPixelShader) != S_OK)
        {
            return false;
        }
    }
...
bool    ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
    ...
    io.BackendRendererName = "direct11";
